...
"""
script to generate PyLadies meetup .csv with the following information:
- meetup name
- meetup id
- meetup url
...
To use you'll need to register a MeetUp app at https://secure.meetup.com/meetup_api/oauth_consumers/ and insert the following values in the `.env` file:
- GITHUB_TOKEN
- MEETUP_CLIENT_ID
    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not completed and response.status_code == 429:
                seconds_until_reset = int(meetup_resp.headers.get('X-RateLimit-Reset'))
                if seconds_until_reset and meetup_resp.json().get('errors')[0].get('code') == 'throttled':
                    print(f'Sleeping {seconds_until_reset} until throttle is complete ...')
                    time.sleep(seconds_until_reset)
                    print(f'Trying again now that throttle is complete ...')
                    completed, response = fn(*args, **kwargs)
...
class MeetUpApi(object):
    ...
    def get_bearer_token(self):
        auth_endpoint = f'{MeetUpApi.api_auth_url}/oauth2/authorize?client_id={self.client_id}&redirect_uri={self.redirect_uri}&response_type=code'
        ...
        try:
            ...
            raise Exception('Unable to obtain MeetUp access token.')

    @ratelimit(number_times=MAX_MEETUP_REQUESTS_PER_HOUR)
    def get_groups(self, topic_ids):
        """
        :param  meetup_ids: list of numeric topic_ids, if unknown use the get_group endpoint
        ...
        try:
            ...
            raise Exception('Exception processing MeetUp data')

    @ratelimit(number_times=MAX_MEETUP_REQUESTS_PER_HOUR)
    def get_group(self, chapter_name):
        """
        :param  chapter_name: meetup chapter urlnames e.g. meetup.com/Chicago-Pyladies/ the urlname is 'Chicago-Pyladies'
...
def download_pyladies_chapters(token):
    ...
    response = requests.get(
        url='https://api.github.com/repos/pyladies/pyladies/contents/www/config.yml',
        headers=headers
    )
...
if __name__ == "__main__":
    if not GITHUB_TOKEN:
        GITHUB_TOKEN = input('To download the PyLadies MeetUp chapters, please provide a GitHub token with repo access privileges.\n')
    ...
    meetup_api = MeetUpApi(client_id=MEETUP_CLIENT_ID, client_secret=MEETUP_CLIENT_SECRET, redirect_uri=MEETUP_REDIRECT_URI)
    ...
    for chapter in chapter_data.get('chapters'):
        ...
        if not completed:
            print(f'Error {meetup_resp.status_code}, skipping chapter {meetup_url_name}')
            continue

        meetup_resp = meetup_resp.json()

        if chapter.get('meetup_id') != meetup_resp.get('id'):
            print(f"MeetUp Chapter {chapter.get('name')} id is wrong {meetup_resp.get('id')}")
        ...
        if meetup_resp.get('group_photo'):
            chapter['meetup_image'] = meetup_resp.get('group_photo').get('highres_link')
